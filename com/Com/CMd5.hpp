...
static sMD5 GetMd5(const char *p,int nLen)
{
	...
	Md5 md5;
	md5.md5_update((uint8*)p,nLen);
	md5.md5_finish(ret.md5);
	for (int i = 0; i < 16; i++)
	{
		sprintf(ret.md5str+ i * 2, "%02X", ret.md5[i]); 
	}
	return ret;
}
...
static sMD5 GetMd5File(const char *pName)
{
	...
	Buf buf = XReadFile(pName);
	if (0 == buf.m_nLen)
	{
		sMD5 ret;
		strcpy(ret.md5str, "empty");
		return ret;
	}
	return GetMd5(buf.m_addr, buf.m_nLen);
